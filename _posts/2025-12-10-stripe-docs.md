...
## 고객 데이터를 Stripe로 마이그레이션
---

고객 데이터를 성공적으로 Strpe로 마이그레이션합니다.

고객 데이터를 Stripe로 마이그레이션하는 것은 여러 단계를 거치는 프로세스입니다. 이 가이드를 읽고나면:

- Stripe 마이그레이션 프로세스를 이해할 수 있습니다.
- 마이그레이션 일정을 계획할 수 있습니다.
- 성공적인 마이그레이션에 필요한 연동 요소를 알 수 있습니다.
- 사용자에게 최소한의 불편을 주면서 결제 정보를 마이그레이션하는 방법을 이해할 수 있습니다.
...
- 신규 고객부터 시작하여 데이터 마이그레이션 계획을 수립합니다. 
- 최종 목표는 신규 고객 100%를 마이그레이션한 후 기존 고객을 마이그레이션하는 것입니다.
- 고객이 카드 정보를 업데이트할 수 있는 프로세스를 설계합니다.
...
- Stripe의 마이그레이션 문서를 검토합니다.
- 이전 프로세서에 연락하여 마이그레이션 프로세스를 파악합니다.

<br/>

### 3단계: 마이그레이션 계획 및 기존 프로세서 연결
---

- 마이그레이션할 결제 정보를 식별합니다.
- 마이그레이션할 결제 수단을 식별합니다.
- 마이그레이션할 고객 레코드 수를 파악합니다.
- 마이그레이션할 고객 레코드 수를 파악합니다.
- 이전 프로세서, 고객 수, 예정된 마감일을 고려하여 마이그레이션 일정을 계획합니다.
- Stripe Migrations 팀에 이전 프로세서, Stripe 계정 번호, 마이그레이션할 레코드 수, 가져올 결제 수단 유형에 대한 세부 정보를 보냅니다.

<br/>

### 4단계: Stripe Migrations 팀
---

- 기존 프로세서를 Stripe의 Migrations 팀에 소개합니다.
- Stripe 또는 기존 프로세서의 마이그레이션 팀이 요청하는 조치 항목을 완료하거나 추가 정보를 제공합니다.
...
- Stripe와 이전 프로세서 간의 커뮤니케이션을 따라 팀이 준비되도록 합니다.
- 마이그레이션 중 확인된 문제에 대응합니다.
- Stripe Migration 팀에서 JSON 매핑 파일이 포함된 이메일을 확인합니다.
- JSON 매핑 파일을 파싱하고 그에 따라 데이터베이스를 업데이트합니다.
- 고객이 카드 정보를 업데이트할 수 있는 프로세스를 구현합니다.
- 리매핑 계획을 설계하고, 해당되는 경우 구독(Subscription은 고객이 구독하는 플랜과 관련된 제품 세부 정보를 나타내며, 반복적으로 고객에게 청구할 수 있음) 리매핑을 포함합니다.
- Stripe에서 기존 고객에게 청구를 시작합니다.
...
민감한 결제 정보를 다른 결제 프로세서로 전송하거나 다른 프로세서로부터 전송받아야 하는 경우, 또는 Stripe 계정 간에 전송해야 하는 경우, 안전하고 PCI 규정을 준수하는(신용카드 데이터 처리, 전송 또는 저장에 관여하는 모든 당사자는 PCI(Payment Card Industry) 데이터 보안 표준에 명시된 규칙을 준수해야 합니다. PCI 규정 준수는 공동 책임이며 Stripe와 귀차힁 비즈니스 모두에 적용됩니다) 방식으로 도움을 드릴 수 있습니다.

고객 레코드와 관련 결제 데이터가 모두 포함된 요청인 경우에만 지원할 수 있습니다. 결제 정보가 포함되지 않는 고객 데이터를 생성, 업데이트 또는 조회하려면 Stripe의 Customer API를 사용하세요.

<br/>

## 사례: 스타트업을 위한 간단한 결제 연동
---

코드를 작성하지 않고 비즈니스에서 Stripe 결제를 활성화하는 방법을 알아보세요.

이 가이드는 스타트업으로서 일회성 결제를 수락하기 위해 Stripe 연동에서 수행해야 하는 작업을 설명합니다.
...
Stripe와 연동하기 전에 Stripe 계정을 생성해야 합니다.

1. 이메일 주소, 성명, 국가를 입력하고 비밀번호를 생성하여 계정을 생성합니다.
2. 비즈니스 프로필을 작성합니다.
3. 대시보드에서 Verify your email을 클릭합니다. 이메일 주소로 인증 이메일이 발송됩니다.
4. 이메일 주소를 인증합니다.
...
Payment Links를 사용하면 온라인 판매 시 고객이 결제할 수 있습니다. 모든 사람과 공유할 수 있는 단일 링크를 생성합니다.

**결제 링크 생성 방법**

1. 대시보드에서 Payment Links 페이지를 열고 +New를 클릭합니다.
2. 기존 제품을 선택하거나 +Add a new product를 클릭합니다.
3. 새 제품을 추가하는 경우, 제품 세부 정보를 입력하고 Add product를 클릭합니다.
4. Create link를 클릭합니다.
...
대시보드를 사용하여 결제 링크를 복사하고 온라인에서 공유합니다. Payment Links 페이지의 기존 링크 옆에 있는 복사 아이콘을 클릭하거나, 결제 링크의 상세 페이지로 이동합니다. 결제 링크는 다음을 포함하여 온라인 어디서든 여러 번 공유할 수 있습니다:
...
1. 대시보드에서 계정 설정을 엽니다.
2. 비즈니스 유형, 세금 정보, 비즈니스 세부 정보, 본인 확인 정보, 고객 대면 정보(ex. 명세서 표시명)를 입력합니다.
3. 대금이 지급될 위치를 확인하기 위해 은행 정보를 추가합니다.
4. 계정을 보호하기 위해 2단계 인증을 설정합니다.
5. 선택적으로 자동 세금 징수 또는 수익 기반 기후 기부를 추가할 수 있습니다.
6. 입력한 정보를 검토하고 Agree and submit을 클릭합니다.
7. 프로필을 활성화하면 Stripe가 샌드박스 모드에서 라이브 모드로 업데이트합니다.
...
연동을 설정한 후 다음 기능을 구현하는 것이 좋습니다:

- 체크아웃 UI 커스터마이징 : 브랜드 설정으로 외관 맞춤화
- 프로모션 코드 생성 : 고객에게 할인 제공
- 주문 이행 관리 : 주문 및 지표 확인
- 버튼으로 결제 링크 삽입 : 웹사이트에 버튼 형태로 삽입

<br/>

## 사례: SaaS 스타트업으로서 구독 판매
---

SaaS 스타트업으로서 사전 구축된 구독 연동을 시작합니다.

이 가이드는 정액제 가격 모델을 사용하여 반복 결제(구독)를 연동하려는 SaaS 스타트업을 위한 것입니다. 이는 특정 기간이 시작될 때 제품이나 서비스에 대해 월별 정액을 청구하는 것을 의미합니다.

고객은 호스팅된 페이지로 리디렉션되어 결제 정보를 입력한 후 사이트로 돌아옵니다. Checkout Sessions API를 호출하는 Stripe 호스팅 페이지를 만들기 위해 Checkout을 사용합니다.
...
대시보드에서 제품을 생성하고 가격을 설정합니다.

1. 대시보드에서 테스트 제품을 생성합니다.
2. 제품 및 가격 세부 정보를 입력합니다. (Name, Price, Currency)
3. 테스트 제품 이미지를 업로드합니다.
4. Recurring을 선택합니다.
5. Add test product를 클릭합니다.
...
결제를 수락하려면 사전 구축된 결제 페이지인 Stripe 호스팅 Checkout으로 연동을 구축합니다.
...
이 단계를 진행하면서 Stripe 호스팅 Checkout 퀵스타트를 참조하세요.
...
서버는 Stripe API와 통신하여 체크아웃 프로세스를 초기화하지만, Stripe가 실제 결제 양식을 안전하게 호스팅합니다. 서버 설정은 프로그래밍 언어에 따라 다르지만, 일반적으로 Stripe 라이브러리 설치, API 키 구성, Checkout Session 생성 및 결제 후 프로세스 처리에 필요한 엔드포인트 구현이 포함됩니다.

1. 퀵스타트에서 프론트엔드, 백엔드 기술 선택합니다.
2. Stripe 라이브러리 설치합니다.
3. Checkout Session을 생성합니다. Checkout Session은 고객의 결제 흐름을 정의하는 Stripe의 서버측 객체입니다. 엔드포인트 경로는 체크아웃 버튼의 `action` 속성과 일치해야 합니다.
4. 판매할 제품을 정의합니다. 대시보드에서 이전에 생성한 테스트 제품을 추가합니다. 테스트 제품을 설정하면 price ID 값을 없으며, 프론트엔드에서 Checkout Session 생성 시 이를 전달해야 합니다.
5. 결제 모드를 선택합니다. 구독으로 반복 결제를 시작하려면 `mode` 를 `subscription` 으로 설정합니다.
6. 성공 URL을 제공합니다. Stripe는 고객이 결제를 완료하거나 취소한 후 이 페이지로 리디렉션합니다. 이 페이지를 사이트에 호스팅하세요.
7. 고객을 Checkout 페이지 URL로 리디렉션합니다. 이는 Checkout Session 응답에서 반환되는 URL입니다.
...
이것은 고객이 결제를 완료하기 전에 주문 요약을 보여주는 사이트 페이지입니다. 필요한 경우 선택 항목을 검토하고 수정할 수 있습니다.

1. 주문 미리보기 페이지를 추가합니다.
2. 체크아웃 버튼을 추가합니다. 이것은 고객이 주문 미리보기 페이지에서 클릭하여 Stripe 호스팅 결제 페이지로 리디렉션되는 버튼입니다.
...
1. 로컬 개발 중에 `package.json` 파일에 `"proxy" : "http://localhost:4242"` 를 추가합니다.
2. `npm start` 를 실행하고 http://localhost:3000/checkout 으로 이동합니다.
3. 체크아웃 페이지에서 테스트 카드 `4242424242424242` 를 입력하여 고객으로서 테스트 결제를 제출합니다.
4. (선택 사항) Link 계정에 가입하려면 Save my information for faster checkout을 선택합니다. 재방문 고객의 체크아웃 흐름을 확인하기 위해 Link를 결제 수단으로 테스트할 수 있습니다.
...
결제 수단 추가, 체험 기간, 고객 포털 설정 방법을 알아보세요.
...
Checkout에 커스텀 브랜딩을 적용할 수 있습니다. 대시보드에서 브랜딩 설정으로 이동하여:

- 로고 또는 아이콘 업로드
- 페이지의 배경색, 버튼, 색상, 폰트, 모양 커스터마이징
...
대시보드의 결제 수단 페이지에서 고객으로부터 수락할 결제 수단을 활성화합니다. Checkout은 여러 결제 수단을 지원하며, 동적 결제 수단도 사용합니다.

이는 Stripe가 전환율을 극대화하기 위해 각 고객에게 가장 관련성 높은 적격 결제 수단을 동적으로 표시하는 로직을 처리한다는 것을 의미합니다. Stripe는 위치나 결제 수단이 구독을 지원하는지 여부와 같은 요소를 기반으로 사용할 수 있는 결제 수단만 고객에게 표시합니다.
...
다음 파라미터 중 하나를 전달하여 고객의 구독에 무료 체험을 추가하도록 Checkout Session을 구성합니다:

- `subscription_data.trial_period_days` : 무료 체험 기간(일 단위)
- `subscription_data.trial_end` : 체험 기간 종료를 나타내는 Unix 타임스탬프
  ...
  SessionCreateParams.builder()
    ...
    .setSubscriptionData(
      SessionCreateParams.SubscriptionData.builder()
        .setTrialPeriodDays(30L) // 30일 무료 체험
        .build()
    )
    .build();
...
고객은 고객 포털에서 기존 구독과 인보이스를 관리합니다. 대시보드를 사용하여 포털을 생성합니다. 최소한 고객이 결제 수단을 업데이트할 수 있도록 구성해야 합니다.
...
Stripe 호스팅 Checkout 연동에 대한 구독을 모니터링하려면, 구독 라이프사이클 전체에서 발생하는 특정 webhook 이벤트를 수신할 엔드포인트를 설정할 수 있습니다.
...
1. Stripe로부터 이벤트 알림을 받기 위해 서버에 webhook 엔드포인트를 생성합니다.
2. 대시보드에서 webhook 설정에 이 엔드포인트를 등록합니다.
3. webhook 시크릿을 사용하여 보안을 위한 webhook 서명 검증을 구현합니다. `STRIPE_WEBHOOK_SECRET` 키를 모르면 Workbench의 Webhooks 탭에서 대상 상세 보기로 이동하여 확인합니다.
4. Workbench에서 원하는 이벤트를 선택합니다.
5. 대시보드에서 엔드포인트의 이름, URL, 설명을 입력합니다.
6. 최소한 다음 이벤트를 모니터링합니다 (더 많은 이벤트는 구독 웹훅 참조)
    1. `checkout.session.completed` 
        1. 설명 : 고객이 Checkout Session을 성공적으로 완료하여 새 구매를 알릴 때 전송
        2. 대응 방법 : entitlements를 사용하여 구독을 프로비저닝
    2. `invoice.paid` : 
        1. 설명 : 결제가 성공할 때마다 각 청구 기간에 전송
        2. 대응 방법 : 월별 청구의 경우 `invoice.paid` 이벤트를 받을 때마다 매월 구독을 계속 프로비저닝. 이벤트 유형을 검사하고 각 이벤트 객체의 페이로드를 파싱. 검증 목적으로 `subscription.id` 와 `customer.id` 이벤트 객체를 데이터베이스에 저장
    3. `invoice.payment_failed`
        1. 설명 : 고객의 결제 수단에 문제가 있을 때 각 청구 기간에 전송
        2. 대응 방법 : 고객에게 알리고 결제 수단을 업데이트하도록 고객 포털로 안내
...
Checkout Session을 생성할 때 구독의 청구 주기 앵커를 고정 날짜(ex. 다음 달 1일)로 명시적으로 설정할 수 있습니다. 청구 주기 앵커는 고객에게 전체 구독 금액이 청구되는 첫 번째 전체 인보이스 날짜를 결정합니다. 예를 들어, 5월 15일에 생성되고 앵커가 6월 1일인 월간 구독은 처음에 5월 15일에 청구된 다음 항상 매월 1일에 청구됩니다.

청구 구지 앵커를 구성하려면 `subscription` 모드에서 Checkout Session을 생성할 때 `subscription_data.billing_cycle_anchor` 파라미터를 설정합니다. 앵커는 다음 자연 구독 청구일 이전의 미래 UNIX 타임스탬프여야 합니다.
...
SessionCreateParams params =
  SessionCreateParams.builder()
    ...
    .setSubscriptionData(
      SessionCreateParams.SubscriptionData.builder()
        .setBillingCycleAnchor(1611008505L) // Unix 타임스탬프
        .build()
    )
    .build();
...
- 청구 주기 앵커가 세션의 활성 시간 동안이고 고객이 앵커가 지난 후에 결제를 시도하면, Checkout은 청구 주기 앵커 이전에 비례 배분 기간 대신 청구 주기 앵커부터 시작하는 전체 기간을 표시하고 청구합니다.
...
1. 세금 자동 징수 활성화 : Checkout 세션에서 구독에 대한 세금 활성화 및 자동 징수
2. 결제 취소 또는 환불 : 완료 전 결제 취소 또는 성공 후 전체/일부 환불
3. 은행 계좌 설정으로 지급금 수령 : 대금 수령을 위한 은행 계좌 설정
4. 적응형 가격 책정 활성화 : 150개 이상 국가에서 고객이 현지 통화로 결제 가능
...
웹사이트에 구독 가겨포를 표시하고 고객을 Stripe Checkout으로 직접 안내합니다.

Stripe 대시보드를 사용하여 임베드 가능한 가격표를 만들 수 있습니다:

- 가격 정보(가격 모델은 판매하는 제품 또는 서비스, 비용, 결제에 허용되는 통화, 청구할 서비스 기간(구독의 경우)으로 구성됩니다. 가격 모델을 구축하려면 제품과 가격을 사용합니다)을 표시하고 고객을 사전 구축된 체크아웃 흐름으로 안내합니다. 체크아웃 흐름은 Stripe Checkout을 사용하여 구매를 완료합니다.
- 정액제, 시트당 가격, 단계별 가격, 무료 체험과 같은 일반적인 구독 비즈니스 모델을 지원합니다.
- 코드를 작성할 필요 없이 대시보드에서 직접 제품 및 가격 정보를 구성, 커스터마이징, 업데이트합니다.
- `<script>` 태그와 웹 컴포넌트로 웹사이트에 삽입합니다. Stripe가 자동으로 태그를 생성합니다. 웹사이트 코드에 복사하여 붙여넣습니다.
...
업데이트 가능한 Stripe 객체(Account, Charge, Customer, PaymentIntent, Refund, Subscription, Transfer)에는 `metadata` 파라미터가 있습니다. 이 파라미터를 사용하여 Stripe 객체에 키-값 데이터를 첨부할 수 있습니다.
...
메타데이터를 사용하여 객체에 추가적인 구조화된 정보를 저장할 수 있습니다.

Stripe는 메타데이터를 사용하지 않습니다.

- 청구를 승인하거나 거부하는 데 사용하지 않음
- 사용자에게 표시하도록 선택하지 않는 한 보이지 않음

<br/>

#### metadata vs description 비교
---

| 특성      | metadata                                      | description                       |
| ------- | --------------------------------------------- | --------------------------------- |
| 형식      | 키-값 쌍 (최대 50개)                                | 단일 문자열                            |
| 사용자 가시성 | 표시하도록 선택하지 않은 한 보이지 않음                        | 사용자에게 보일 수 있음 (ex. 이메일 영수증)       |
| 용도      | 시스템 연동, 내부 추적                                 | 사람이 읽을 수 있는 주석                    |
| 예시      | `{"order_id" : "6735", "user_id" : "abc123"}` | `"2 shirts for test@example.com"` |
- 민감한 정보를 메타데이터나 `description` 파라미터에 저장하지 마세요.
...
- ID 연결 : 시스템의 고유 ID를 Stripe 객체에 첨부하여 조회 간소화. 예: 청구에 주문 번호 추가, 고객에 사용자 ID 추가, 이체에 고유 영수증 번호 추가
- 환불 기록 : 환불 이유와 생성 담당자에 대한 정보 저장
- 고객 세부 정보 : 향후 사용을 위해 내부 ID로 고객에 주석 달기
...
Customer customer = client.v1().customer().create(params);
```
...
모든 최상위 API 리소스는 "list" API 메서드를 통한 대량 가져오기를 지원합니다. 예를 들어 청구 목록, 고객 목록, 인보이스 목록을 가져올 수 있습니다. 이러한 list API 메서드는 공통 구조를 공유하며 최소한 `limit` , `starting_after` , `ending_before` 세 가지 파라미터를 받습니다.

Stripe의 list API 메서드는 `starting_after` 및 `ending_before` 파라미터를 통해 커서 기반 페이지네이션을 사용합니다. 두 파라미터 모두 기존 객체 ID 값을 받고 객체를 역시간순으로 반환합니다. `ending_before` 파라미터는 지정된 객체 이전에 나열된 객체를 반환합니다. `starting_after` 파라미터는 지정된 객체 이후에 나열된 객체를 반환합니다. 이러한 파라미터는 상호 배타적입니다. `starting_after` 또는 `ending_before` 파라미터 중 하나를 사용할 수 있지만 동시에 둘 다 사용할 수는 없습니다.
...
| 파라미터             | 유형            | 설명                                                                                                                                       |
| ---------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `limit`          | 선택 사항, 기본값 10 | 반환할 객체 수 제한 지정, 1 에서 100 사이                                                                                                              |
| `starting_after` | 선택 사항, 객체 ID  | 페이지네이션에 사용할 커서, 목록에서 위치를 정의하는 객체 ID. 예: list 요청을 하고 `obj_foo` 로 끝나는 100개 객체를 받으며 후속 호출에 `starting_after=obj_foo` 를 포함하여 다음 페이지를 가져올 수 있음 |
| `ending_before`  | 선택 사항, 객체 ID  | 페이지네이션에 사용할 커서, 목록에서 위치를 정의하는 객체 ID. 예: list 요청을 하고 `obj_bar` 로 시작하는 100개 객체를 받으면 후속 호출에 `ending_before=obj_bar` 를 포함하여 이전 페이지를 가져올 수 있음 |
...
- 1차 요청: GET /v1/customers?limit=3
	- 응답: \[obj_a, obj_b, obj_c], has_more: true
- 2차 요청: GET /v1/customers?limit=3&starting_after=obj_c
	- 응답: \[obj_d, obj_e, obj_f], has_more: true
- 3차 요청: GET /v1/customers?limit=3&ending_before=obj_d
	- 응답: \[obj_a, obj_b, obj_c], has_more: ...
...
| 필드         | 유형      | 설명                                               |
| ---------- | ------- | ------------------------------------------------ |
| `object`   | string  | 반환되는 객체 유형에 대한 설명을 제공하는 문자열. 값은 `"list"`         |
| `data`     | array   | 요청 파라미터로 페이지네이션된 실제 응답 요소를 포함하는 배열               |
| `has_more` | boolean | 이 집합 이후에 더 많은 요소가 있는지 여부. `false` 이면 이 집합이 목록의 끝 |
...
| 특성      | v1 API                                 | v2 API      |
| ------- | -------------------------------------- | ----------- |
| 커서 파라미터 | `starting_after`, `ending_before`      | 다른 인터페이스 사용 |
| 제한 파라미터 | `limit` (1-100)                        | 다름          |
| 응답 형식   | `object` , `data` , `has_more` , `url` | 다른 형식       |
...
일부 최상휘 API 리소스는 "search" API 메서드를 통한 검색을 지원합니다. 예를 들어 청구 검색, 고객 검색, 구독 검색을 할 수 있습니다.

Stripe의 search API 메서드는 `page` 요청 파라미터와 `next_page` 응답 파라미터를 통해 커서 기반 페이지네이션을 활용합니다. 예를 들어 검색 요청을 하고 싶어서 `"next_page": "pagination_key"` 를 받으면 후속 호출에 `page=pagination_key` 를 포함하여 다음 페이지의 결과를 가져올 수 있습니다.

클라이언트 라이브러리는 검색 결과의 모든 페이지를 쉽게 순회하기 위한 자동 페이지네이션 헬퍼를 제공합니다.

<br/>

**Search vs List 비교**

| 특성      | List API                           | Search API                   |
| ------- | ---------------------------------- | ---------------------------- |
| 목적      | 모든 객체 나열                           | 쿼리 기반 필터링                    |
| 커서 파라미터 | `starting_after` , `ending_before` | `page`                       |
| 응답 커서   | 마지막 객체 ID 사용                       | `next_page`                  |
| 응답 객체   | `"object" : "list"`                | `"object" : "search_result"` |
| 총 개수    | 미제공                                | `total_count` (expand 필요)    |
| 쿼리 언어   | 없음                                 | 검색 쿼리 언어                     |
...
| ------- | ----- | -------------------------------------------------------------------------------------------- |
| `query` | 필수    | 검색 쿼리 문자                                                                                      |
| `limit` | 선택 사항 | 반환되는 객체 수 제한, 1에서 100 사이, 기본값                                                                 |
| `page`  | 선택  여러 결과 페이지에 걸친 페이지네이션 커서. 첫 번째 호출에는 이 파라미터를 포함하지 않음. 이전 응답에서 반환된 `next_page` 값을 사용하여 후속 결과 요청 반환된  |
...
| 필드            | 유형                | 설명                                                                                    |
| ------------- | ----------------- | ------------------------------------------------------------------------------------- |
| `object`      | string            | 반환되는 객체 유형을 설명하는 문자열. 값은 `"search_result"`                                            |
| `url`         | string            | 이 목록에 접근하기 위한 URL                                                                     |
| `has_more`    | boolean           | 이 집합 이후에 더 많은 요소가 있는지 여부. `false` 이면 이 집합이 목록의 끝                                      |
| `data`        | array             | 요청 파라미터로 페이지네이션된 실제 응답 요소를 포함하는 배열                                                    |
| `next_page`   | string            | 페이지네이션에 사용할 거서. `has_more` 가 true이면 `next_page` 값을 후속 호출에 전달하여 다음 결과 페이지를 가져올 수 있음    |
| `total_count` | 선택 사항, 양의 정수 또는 0 | 쿼리와 일치하는 객체의 총 수, 최대 10,000까지만 정확함. 기본적으로 포함되지 않음. 응답에 포함하려면 `total_count` 필드를 expand |

<br/>

**예시**

GET /v1/customers/search?query=email:"test@ex.com"
...
| 사용 사례      | 쿼리 예시                                                      |
| ---------- | ---------------------------------------------------------- |
| 이메일로 고객 검색 | `email:"test@example.com"`                                 |
| 메타데이터로 검색  | `metadata["order_id"]:"6735"`                              |
| 생성 날짜로 검색  | `created>1609459200`                                       |
| 여러 조건 AND  | `email:"test@example.com" AND metadata["status"]:"active"` |
| 여러 조건 OR   | `email:"test@example.com" OR email:"other@example.com"`    |
| 부분 일치      | `name~"John"`                                              |
...
| 리소스            | 검색 엔드포인트                     |
| -------------- | ---------------------------- |
| Charges        | `/v1/charges/search`         |
| Customers      | `/v1/customers/search`       |
| Invoices       | `/v1/invoices/search`        |
| PaymentIntents | `/v1/payment_intents/search` |
| Prices         | `/v1/prices/search`          |
| Products       | `/v1/products/search`        |
| Subscriptions  | `/v1/subscriptions/search`   |
...
Stripe 라이브러리는 자동 페이지네이션을 지원합니다. 이 기능을 사용하면 후속 페이지를 가져오기 위한 수동 요청 없이 대량의 리소스 목록을 쉽게 순회할 수 있습니다.
...
Iterable<Customer> itCustomers = 
  Customer.list(customerParams).autoPagingIterable();
...
### 연결된 계정 (Connected Accounts)
---

Stripe Connect를 사용하는 경우 연결된 계정을 대신하여 요청을 발행할 수 있습니다. 연결된 계정으로 작업하려면 일반적으로 `acct_` 접두사로 시작하는 연결된 계정 ID가 포함된 `Stripe-Account` 헤더를 포함합니다.

연결된 계정 ID는 요청별로 설정됩니다. 반환된 객체의 메서드는 동일한 계정 ID를 재사용합니다.
