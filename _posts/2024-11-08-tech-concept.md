...
**오픈 소스 인메모리 데이터 구조 저장소**로, 데이터베이스, 캐시, 메시지 브로커 등 다양한 용도로 활용된다. 데이터를 **메모리에 저장하여 매우 빠른 성능을 제공**하며, 문자열, 리스트, 해시, 셋, 정렬된 셋 등 **다양한 데이터 구조를 지원**한다. 또한, 복제, 클러스터링, 지속성 오션 등을 통해 **고가용성과 확장성을 제공**한다.[^Redis]
