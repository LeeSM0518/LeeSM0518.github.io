...
* **Process(프로세스)** 
  * 실행중인 프로그램을 뜻한다.
  * 프로세스들은 작업을 완수하기 위해 자원들을 필요로 함.
    * ex) CPU, 메모리 파일, 입출력 장치 등
  * 작업의 단위(the unit of work) 이다.
...
* **프로그램(수동적) != 프로세스(능동적)**
* 설치(프로그램) => 실행(프로세스)
* 내용(프로그램) => 메모리에 올림, 실행중인 프로세스의 위치 정보, 레지스터 활용(프로세스)
...
* **프로세스** = 실행중인 프로그램
  ...
  * **시스템의 작업 단위** ,  모든 CPU 활동에 대한 이름
  * **능동적인 존재(active entity)**
    * PC(Program Count), 레지스터의 내용으로 대표되는 현재 활동을 포함
    * 사용자 정의 변수들을 포함한다.
  * 두 프로세스가 동일해도 두 개의 분리된 실행 순서를 가진다.
...
> 4. **대기(waiting)** : 입출력 후 대기 상태가 된다.
> 5. **인터럽트(interrupt)** : 프로세스를 중단시킨 뒤, 준비완료 상태로 보낸다.
> 6. **종료(terminated)** : 실행이 완료되면 종료 상태로 프로세스를 끝낸다.
>
...
* **프로세스 제어 블록 (Process Control Block: PCB)**
  ...
  : 각 프로세스는 운영체제에서 프로세스 제어 블록(태스크 제어 블록)에 의해 표현된다.

  * 프로세스에 대한 정보를 가진 데이터 블록
  * 프로세스의 모든 정보에 대한 기억 장소 역할을 한다.
...
<img src="../capture/스크린샷 2019-04-08 오후 12.32.29.png" width="600">

> 1. 프로세스 P0, **running** (CPU에서 실행중)
> 2. 프로세스 P0, **waiting** (입출력) PCB0 에 상태 저장
> 3. PCB1 으로부터 상태를 프로세스 P1에 적재, 프로세스 P1 **running**
...
* **다중 프로그래밍(multi programming)** 은 CPU 이용을 최대화하기 위하여 항상 어떤 프로세스가 실행되도록 한다.
* **시분할** 은 각 프로그램이 실행되는 동안 사용자가 상호 작용할 수 있도록 프로세스들 사이에서 CPU를 빈번하게 교체한다.
* **프로세스 스케줄러(process scheduler)** : 수행 가능한 여러 프로세스들 중에서 하나의 프로세스를 선택한다.
...
* **작업 큐(job queue)** : 메인 메모리의 할당을 기다리면서 기억장치에 있는 모든 프로세스들로 구성
* **준비 완료 큐(ready queue)** : 메인 메모리에 존재하며, 준비 완료 상태에서 실행을 대기하는 프로세스들로 구성
...
* 프로세스는 일생 동안 **다양한 스케줄링 큐들 사이를 이주하며 어떤 방식으로든지 스케줄링 목적을 위해 적절한 스케줄러에 의하여 선택된다.**
...
* **장기 스케줄러는 신중한 선택을 하는 것이 중요하다.**
  * 입출력 중심 프로세스 (I/O-bound process) : 입출력 수행을 하는데 시간을 소요
  * CPU 중심 프로세스 (CPU-bound process) : 연산을 수행하는데 시간을 소요
...
* **문맥(context)은 프로세스의 PCB에 표현된다.**
  ...
  * 문맥(context) : 현재 올라와 있는 프로세스의 정보들
  * CPU 레지스터 값, 프로세스 상태, 메모리 관리 정보 등

* **문맥 교환(context switch)**

  * CPU를 다른 프로세스로 교환하기 위해 이전의 **프로세스의 상태를 PCB에 저장하고 새로운 프로세스의 저장된 상태를 PCB에서 복구** 하는 작업.
  * 문맥 교환 시간은 **하드웨어에 의해 좌우된다.** (특수명령어: 문맥 교환을 한번에 실행시키는 명령어)
  * 문맥 교환 시간은 **순수한 오버헤드이다.** (CPU 입장에서 문맥교환을 할 때 시간이 낭비된다.)
    * 문맥 교환이 진행될 동안 시스템은 아무런 일을 하지 못한다.
...
* 자식 프로세스는 생성될 때 자신의 임무를 달성하기 위해 **자원이 필요하다.**
  * 자원을 **운영체제로부터** 직접 얻거나,
...
* 프로세스는 **exit 시스템 호출을** 사용하여 운영 체제에 자신의 삭제를 요청하면 종료된다.
  ...
  * 프로세스의 모든 자원은 운영 체제로 **반납된다.**
* 프로세스는 시스템 호출을 통해서 다른 프로세스의 **종료를 유발할(kill)** 수 있다.
  * 종료될 프로세스의 부모만이 가능.
* 부모 프로세스는 여러 가지 이유로 **자식 중 하나의 실행을 종료할 수 있다.**
  * 자식 프로세스가 할당된 자원을 초과하여 사용할 때
  * 자식에게 할당된 태스크가 더 이상 필요 없을 때
  * 부모가 종료되면 자식이 계속 수행되는 것을 허용하지 않을 때
...
* **프로세스 협력을 허용하는 환경을 제공하는 이유**
  * **정보 공유** (Information sharing) : 정보를 병행적으로 접근할 수 있도록 한다.
  * **계산 가속화** (Computation speed-up) : 각각 다른 서브 태스크들과 병렬로 실행할 수 있다.
  * **모듈성** (Modularity) : 모듈 형태로 시스템 구성한다.
...
* 통신하는 프로세스들이 **공유 메모리 영역을 구축** 해야 한다.
  * 통상 공유 메모리 영역은 **공유 메모리 세그먼트** 를 생성하는 프로세스의 주소 공간에 위치한다.
  * 다른 프로세스가 이 공유 메모리 세그먼트를 이용하여 통신하고자 하면 이 **세그먼트를 자신의 주소 공간에 추가** 하여야 한다.
* **생산자-소비자 프로세스 예**
  * 생산자(데이터 생산) => 소비자(데이터 소비)

    : 생산자가 정보를 채워 넣고 소비자가 비울 수 있는 **버퍼들의 저장소(pool)를** 제공해야 한다.

  * **무한** 버퍼 (unbounded buffer)의 생산자 - 소비자 문제 

    * 버퍼의 크기에 실질적인 한계가 없다.
    * 소비자가 새로운 항목을 기다려야만 할 수도 있고, 생산자가 항상 새로운 항목을 생산할 수도 있다.

  * **유한** 버퍼 (bounded buffer)의 생산자 - 소비자 문제
...
* **메시지 전달 시스템**
  * **주소 공간을 공유하지 않고도** 프로세스들이 통신을 하고, 그들의 **동작을 동기화할 수 있도록** 허용하는 기법
  * 두 가지 연산 제공
    * send(message)
    * receive(message)
  * 프로세스 간에 **통신 연결(Communication Link)이** 설정되어야 한다.
...
* **직접 통신**
  ...
  * 연결은 두 프로세스 사이에만 **명시적으로 지정됨.** 즉, 하나의 연결이 존재하므로 **두 개의 프로세스만 통신 가능**
* **간접 통신**
  * 메시지들은 **메일 박스(ports)로** 송신되고 그것으로부터 수신된다.
  * **ex) send(A, message), receive(A, message)**
  * 한 쌍의 프로세스들 사이의 연결은 공유 메일 박스를 가질 때만 구축됨
  * 두 개 이상의 프로세스와 연관될 수 있음. 즉, **여러 개의 프로세스가 통신 가능**
  * 각 프로세스의 연결은 하나의 **메일 박스를 할당한다.**
...
* **무용량**(zero capacity)
  ...
  * 두 프로세스간에 메시지 전달을 위해 **동기화가 필요.**
* **유한 용량**(bounded capacity)
  * 큐의 길이가 **n > 0** 인 경우
  * 송신자는 큐가 꽉 찼는지 확인한 뒤 메시지를 큐에 넣거나 기다린다.
* **무한 용량**(unbounded capacity)
  * 큐의 길이가 **무한**
  * 송신자는 기다림 없이 큐에 메시지를 계속 넣는다.
...
네트워크에 연결되어 있는 두 시스템 사이의 통신에 사용하기 위하여 **프로시저 호출 기법** 을 분리해 생각하기 위한 방편으로 설계되었다.

![img](../capture/스크린샷 2019-04-10 오후 11.58.04.png)

* **Remote Procedure Call(RPC: 원격 프로시저 호출)**
  * 클라이언트가 자기의 프로시저를 호출하는 것 처럼 **원격 호스트의 프로시저를 호출하여 사용한다.**
